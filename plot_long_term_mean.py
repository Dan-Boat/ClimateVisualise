...
TO DO: 1. add argument parser to customize more options for calling the functions on command line 
       2. add more plotting variables (wind velocity, humidity, etc (decide on thier colormap))
------------------------------------------------------------
...
def plot_annual_mean(variable, data, title=None , vmin=None, vmax=None , levels=None, 
    ...
    if variable == "Temperature":   # the colormap used in the NCL script was jet_r
        if all(parameter is not None for parameter in [vmin, vmax, levels, center, norm]):    # if all these params are not None 
            p = data.plot.imshow(ax =ax, cmap=plt.cm.RdBu_r, vmin=vmin, vmax=vmax, center=center, levels=levels, transform = ccrs.PlateCarree(), 
                                 norm=norm, cbar_kwargs= {"pad":0.1, "drawedges": True, "orientation": "horizontal", "shrink": 0.60, "format": "%.0f"},
                                 extend= "neither")
        else:
            p = data.plot.imshow(ax=ax, cmap=plt.cm.RdBu_r, vmin=vmin, vmax=vmax, center=0, levels=30, transform = ccrs.PlateCarree(),
                                 cbar_kwargs= {"pad":0.1, "drawedges": True, "orientation": "horizontal", "shrink": 0.60,
                                               "format": "%.0f"})
        ...
        if all(parameter is not None for parameter in [vmin, vmax, levels]):
            p = data.plot.imshow(ax =ax, cmap=plt.cm.Blues, vmin=vmin, vmax=vmax, levels=levels, transform = ccrs.PlateCarree(), 
                                 cbar_kwargs= {"pad":0.1, "drawedges": True, "orientation": "horizontal", "shrink": 1.10, "format": "%.0f", "ticks": ticks},
                                 extend= "neither")
        else:
            p = data.plot.imshow(ax=ax, cmap=plt.cm.Blues, vmin=0, vmax=vmax, levels=30, transform = ccrs.PlateCarree(),
                                 cbar_kwargs= {"pad":0.1, "drawedges": True, "orientation": "horizontal", "shrink": 1.10,
                                               "format": "%.0f", "ticks": ticks}, extend= "neither")
    ...
    if output_format is not None:
        plt.savefig(os.path.join(plot_path, output_name_annual_mean + output_format), bbox_inches="tight")
    elif output_format is None:
        plt.savefig(os.path.join(plot_path, output_name_annual_mean), format="pdf", bbox_inches="tight")
...
def plot_monthly_mean(variable, data, months, title=None , vmin=None, vmax=None , levels=None, 
                     center=None, output_format=None, projection=None, domain = None, norm=None):
    """
    This function generates the plot for long-term monthly mean of a certian variable
    Parameters
    ----------
    variable : TYPE = Str
        DESCRIPTION: Variable to be ploted (imported from options)
    ...
    fig, ((ax1,ax2),(ax3,ax4), (ax5,ax6)) = plt.subplots(3, 2, sharex = True, sharey = True, figsize = (15, 15), 
                                                         subplot_kw={"projection": projection})    
    #plt.subplots_adjust(left=0.05, right=0.8, top=0.94, bottom=0.06)
    ...
    for i in range(len(months_num)):
        cbar_ax = fig.add_axes([0.85, 0.30, 0.03, 0.5])   # axis for subplot colorbar 
        
        if variable == "Temperature":
            if axes[i]==ax5:
                
                if all(parameter is not None for parameter in [vmin, vmax, levels, center, norm]):    # if all these params are not None 
                    p = data[months_num[i]].plot.imshow(ax =axes[i], cmap=plt.cm.RdBu_r, vmin=vmin, vmax=vmax, center=center, levels=levels, transform = ccrs.PlateCarree(), 
                    ...
                    p = data[months_num[i]].plot.imshow(ax=axes[i], cmap=plt.cm.RdBu_r, vmin=vmin, vmax=vmax, center=0, levels=30, transform = ccrs.PlateCarree(),
            ...
            if axes[i]==ax5:
                ...
                if all(parameter is not None for parameter in [vmin, vmax, levels]):    # if all these params are not None 
                    p = data[months_num[i]].plot.imshow(ax =axes[i], cmap=plt.cm.Blues, vmin=vmin, vmax=vmax, levels=levels, transform = ccrs.PlateCarree(), 
                                         cbar_kwargs= {"pad":0.1, "drawedges": True, "orientation": "vertical", "shrink": 0.3, "format": "%.0f"},
                                         extend= "neither", add_colorbar = True, cbar_ax = cbar_ax)
                else:
                    p = data[months_num[i]].plot.imshow(ax=axes[i], cmap=plt.cm.Blues, vmin=0, vmax=1000, levels=30, transform = ccrs.PlateCarree(),
                                         cbar_kwargs= {"pad":0.1, "drawedges": True, "orientation": "vertical", "shrink": 0.3,
                                                       "format": "%.0f"}, add_colorbar = True, cbar_ax = cbar_ax, extend = "neither")
                p.colorbar.set_label(label="Precipitation" + " [" + colorbar_unit + "]", size = 15, 
                                     weight = "bold")  
    ...
    if title is not None:
        fig.suptitle(title_month_mean, fontsize= 20, weight = "bold")
    elif title is None: 
    ...
    if output_format is not None:
        plt.savefig(os.path.join(plot_path, output_name_month_mean + "_" + months + output_format), bbox_inches="tight")
    elif output_format is None:
        plt.savefig(os.path.join(plot_path, output_name_month_mean + "_" + months), format="pdf", bbox_inches="tight")
...
if variable == "Temperature":
    plot_annual_mean(variable, data=data_temp_ltmean, title=title_annual_mean , vmin=vmin, vmax=vmax , levels=levels, 
                         center=center, output_format=output_format, projection=projection, domain=domain, norm= norm) 
    plot_monthly_mean(variable, data=data_temp, months= months, title=title_month_mean , vmin=vmin, vmax=vmax , levels=levels, 
                         center=center, output_format=output_format, projection=projection, domain=domain, norm= norm)
elif variable == "Precipitation": 
    plot_annual_mean(variable, data=data_prec_ltmean, title=title_annual_mean , vmin=vmin, vmax=vmax , levels=levels, 
                         center=center, output_format=output_format, projection=projection, domain=domain, norm= norm)
    plot_monthly_mean(variable, data=data_prec_ltmean, months= months, title=title_month_mean , vmin=vmin, vmax=vmax , levels=levels, 
                         center=center, output_format=output_format, projection=projection, domain=domain, norm= norm)
